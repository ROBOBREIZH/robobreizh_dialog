...
class IntentCommonsense(object):
    """docstring for IntentCommonsense"""
    def __init__(self):
        super(IntentCommonsense, self).__init__()
        self.device = 'cpu'
        self.model_atomic = "pretrained_models/atomic_pretrained_model.pickle"
        self.model_conceptnet = "pretrained_models/conceptnet_pretrained_model.pickle"

        self.categories_custom = ['xAttr', 'xEffect', 'xIntent', 'xNeed', 'xReact', 'xWant']
        self.relation = ['UsedFor', 'IsA']
        self.sampling_custom = ["greedy", "topk-5"]
        self.input_event = "start"
        self.sampling = self.sampling_custom[0]  # greedy OR topk-NUMBER OR beam-NUMBER

        self.params = []
        self.model_dict = {"atomic": self.model_atomic, "conceptnet": self.model_conceptnet}
        for data_name, model in self.model_dict.items():
            self.params.append(self.prepare_interaction(model, self.device, data_name))
        
        self.sampling = "help"
        self.sampling_algorithm = self.sampling

        self.sampler_at = interactive.set_sampler(self.params[0][0], self.sampling_algorithm, self.params[0][1])
        self.sampler_ct = interactive.set_sampler(self.params[1][0], self.sampling_algorithm, self.params[1][1])

        self.category = self.categories_custom
    ...
    def get_conceptnet_sequence(self, e1, model, sampler, data_loader, text_encoder, relation, force=False):
        if isinstance(relation, list):
            outputs = {}

            for rel in relation:
                new_outputs = self.get_conceptnet_sequence(
    ...
    def prepare_interaction(self, model_pretrained, device, data_name):
    ...
    def predict(self, input_sentence):
        outputs_at = self.get_atomic_sequence(input_sentence, self.params[0][3], self.sampler_at, self.params[0][1], self.params[0][2], self.category)
        outputs_ct = self.get_conceptnet_sequence(input_sentence, self.params[1][3], self.sampler_ct, self.params[1][1], self.params[1][2],
                                             self.relation)

        result = {"relation":  outputs_at['xAttr']['beams'][0],
            "intends":outputs_at['xIntent']['beams'][0],
            "desire":outputs_at['xWant']['beams'][0],
            "needs":outputs_at['xNeed']['beams'][0],
            "utility":outputs_ct['UsedFor']['beams'][0],
            "type":outputs_ct['IsA']['beams'][0]}
...
if __name__ == '__main__':
    ...
    for data_name, model in model_dict.items():
        params.append(intents.prepare_interaction(model, device, data_name))
    ...
    while 1:
        ...
        outputs_at = intents.get_atomic_sequence(input_event, params[0][3], sampler_at, params[0][1], params[0][2], category)
        outputs_ct = intents.get_conceptnet_sequence(input_event, params[1][3], sampler_ct, params[1][1], params[1][2],
                                             relation)
