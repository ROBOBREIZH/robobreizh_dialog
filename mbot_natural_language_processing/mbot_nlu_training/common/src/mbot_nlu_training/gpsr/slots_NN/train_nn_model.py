...
class slot_training_class():
    ...
    def __init__(self, yaml_dict):
        ...
        self.resample_replace = yaml_dict['resample_replace']                          # to use or not use data duplication while shuffling after each epoch
    ...
    def shuffle_n_batch(self, data_inputs, data_outputs, lengths):
        ...
        split_list_outputs = []
        split_list_lengths = []
        # shuffling three lists in parallel
    ...
    def recurrent_neural_network(self, rnn_inputs, sequence_lengths):
        ...
        def cell(): return tf.compat.v1.nn.rnn_cell.BasicLSTMCell(self.rnn_size, forget_bias=self.forget_bias)
        # initializing weights and biases for the output layer
        num_layers = self.n_lstm_layers
        layer = {'weights': tf.Variable(tf.random.normal(shape=[self.batch_size, 2*self.rnn_size, self.n_classes]), name='WEIGHTS'),
                 'biases': tf.Variable(tf.random.normal(shape=[self.n_classes]), name='BIASES')}

        # histograms for TB
        if self.use_tensorboard:
            tf.compat.v1.summary.histogram('weights', layer['weights'])
            tf.compat.v1.summary.histogram('biases', layer['biases'])

        # n layer of lstm with rnn_size number of cells for both forward and backward hidden layers.
        with tf.device('/GPU:0'):
            fw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([cell() for num in range(num_layers)], state_is_tuple=True)
            bw_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([cell() for num in range(num_layers)], state_is_tuple=True)
    ...
    def train_neural_network(self):
        ...
        if self.train_method=='new':
            ...
            shuffled_dataset_train = dataset_train.apply(tf.data.experimental.shuffle_and_repeat(buffer_size=self.shuffle_buffer, count=self.repeat_dataset, seed=self.shuffle_buffer))
            ...
            iterator = tf.compat.v1.data.Iterator.from_structure(batched_dataset_train.output_types, batched_dataset_train.output_shapes)
        ...
        ########################################################
        if self.train_method=='old':
            input_placeholder = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size, self.n_steps], name='input_placeholder')
            labels_placeholder = tf.compat.v1.placeholder(dtype=tf.int32, shape=[self.batch_size*self.n_steps, self.n_classes], name='labels_placeholder')
            sequence_length_placeholder = tf.compat.v1.placeholder(dtype=tf.int32, shape=(None), name='inputs_length')
        ...
        embeds = tf.convert_to_tensor(value=word_vectors, name="embeds")
        print('Done')

        # output of the TF graph
        rnn_inputs = tf.nn.embedding_lookup(params=embeds, ids=input_indexes)
        prediction = self.recurrent_neural_network(rnn_inputs, sequence_lengths)

        # defining cost
        with tf.compat.v1.name_scope('COST'):
            cost = tf.reduce_mean(input_tensor=tf.compat.v1.losses.sigmoid_cross_entropy(logits = prediction, multi_class_labels = labels_placeholder))
            tf.compat.v1.summary.scalar('cross_entropy/loss', cost)
        # defining optimizer
        with tf.compat.v1.name_scope('TRAIN'):
            optimizer = tf.compat.v1.train.AdamOptimizer(self.learning_rate).minimize(cost)
        ...
        with tf.compat.v1.name_scope('ACCURACY'):
            ...
            tf.compat.v1.summary.scalar('train dataset accuracy', accuracy)
        ...
        if self.use_tensorboard:
            ...
            writer = tf.compat.v1.summary.FileWriter('./tf_log/validation_accuracy')
        ...
        with tf.compat.v1.Session(config=config) as sess:
            ...
            for epoch in range(self.n_epochs):
                ...
                with tf.compat.v1.name_scope('ACCURACY'):
                    tf.compat.v1.summary.scalar('validation dataset accuracy', p_accuracy)
...
if __name__ == "__main__":
    ...
    yaml_dict = yaml.load(open(config_file))['slots_train']
