...
class intent_training_class():
    ...
    def __init__(self, yaml_dict):
        ...
        self.use_tensorboard = yaml_dict['use_tensorboard']                            # to use or not use tensor board
    ...
    def import_data(self, debug=False):
        ...
        if debug:
            ...
            print('=========================================================')
            print('output labels are {}'.format(outputs[0:2]))        word_vectors, data_inputs, data_outputs, lengths = self.import_data(debug=self.debug)
    ...
    def suffle_n_batch(self, data_inputs, data_outputs, lengths):
        '''
        shuffles and splits the data inputs for each epoch in to batches. 8 batches are used for training and 1 batch is used for validation
        '''
        split_list_inputs = []
        split_list_outputs = []
        split_list_lengths = []
    ...
    def recurrent_neural_network(self, rnn_inputs, sequence_length_placeholder):
        ...
        layer = {'weights': tf.Variable(tf.random.normal([self.batch_size, self.rnn_size, self.n_classes]), name='weights'),
                 'biases': tf.Variable(tf.random.normal([self.n_classes]), name='biases')}

        # n layer of lstm with rnn_size number of cells
        with tf.device('/GPU:0'):
            layers = self.n_lstm_layers
            cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([cell() for _ in range(layers)], state_is_tuple=True)

        # building the graph while running the session (tf methods which reduces the load on the pc)
        outputs, _ = tf.compat.v1.nn.dynamic_rnn(cell, rnn_inputs, sequence_length_placeholder, dtype=tf.float32)
    ...
    def train_neural_network(self):
        ...
        ########################################################
        input_placeholder = tf.compat.v1.placeholder(tf.int32, [self.batch_size, self.n_steps], name='input_placeholder')
        labels_placeholder = tf.compat.v1.placeholder(tf.int32, [self.batch_size, self.n_classes], name='labels_placeholder')
        sequence_length_placeholder = tf.compat.v1.placeholder(tf.int32, [None], name='inputs_length')
        ...
        rnn_inputs = tf.nn.embedding_lookup(params=embeds, ids=input_placeholder)
        ...
        print(prediction)
        # cost function
        cost = tf.reduce_mean(input_tensor=tf.compat.v1.losses.softmax_cross_entropy(logits = prediction, onehot_labels = labels_placeholder))
        ...
        if self.use_tensorboard:
            ...
            writer = tf.compat.v1.summary.FileWriter('./tf_log_intent/')

        # configuring session to use only required amount of memory
        saver = tf.compat.v1.train.Saver()                # for saving the classifiers
        config = tf.compat.v1.ConfigProto()               # for session configurations (eg: gpu memory growth)
        ...
        with tf.compat.v1.Session(config=config) as sess:
            ...
            for epoch in range(self.n_epochs):
                ...
                correct = tf.equal(tf.argmax(input=prediction, axis=1), tf.argmax(input=labels_placeholder, axis=1))
